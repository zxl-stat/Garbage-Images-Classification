...
def main():
    ...
    garbage_model.compile(loss=keras.losses.categorical_crossentropy,
                          optimizer=keras.optimizers.Adam(), metrics=['accuracy'])

    garbage_model.summary()

    early_stop = keras.callbacks.EarlyStopping(monitor='val_accuracy', patience=100)
    garbage_model.fit(x_train, y_train, batch_size=batch_size, epochs=epochs, verbose=1,
